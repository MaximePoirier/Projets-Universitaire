...
int s_cancel(int n, int argc)
{
	...
	char *who, *flags;
		...
		*/
		...
		mdb("cancel: wrong number of parz");
	}
	return 0;
}
	...
	if ( argc == 3 )
	{
		...
		*/
		...
		if (!group_exists) {
			...
			if ( strcasecmp (IDLE_GROUP, n_g_n) == 0 )
			{
	    ...
	    */
		...
		if (ogi < 0) {
			sprintf(mbuf, "[ERROR] Can't find group %s\n", 
				u_tab[n].group);
			mdb(mbuf);
			}
	    }
	    ...
	    mdb("change: wrong number of parz");
	    return (-1);
        }
	return 0;
}
		...
		*/
		...
		if (!is_moderator && is_restricted)
			senderror(n, "You aren't the moderator.");
                ...
                mdb("invite: wrong number of parz");
        }
	return 0;
}
...
extern void add_item(int n, char* item, char *buf);
...
int s_status(int n, int argc)
{
	...
	else
	{
	     ...
	     * trying to change special groups ("1" and IDLE_GROUP)
	     */
		...
		if ( process == 1 )
		{
		    ...
		    case SET_SIZE:
			{
				...
				else
				{
				    ...
				    if ( new_size == 0 )
				    {
					cp = "Group size limit has been removed.";
				    }
				    else
				    {
					sprintf (cp2,
					    "Group is now limited to %d users.",
					    g_tab[gi].size);
					cp = cp2;
				    }
				}
		    ...
		    case SET_IDLEMOD:
			{
			    ...
			    else
				{
					...
					{
						sprintf(mbuf, "Idle-Mod must be between 1 and %d.",
							(MAX_IDLE_MOD/60));
						senderror(n,mbuf);
						cp = NULL;
					}
...
*/
	...
	mdb("status: wrong number of parz");
    }
    return 0;
}
        ...
        if (argc == 2) {
			...
			if (g_tab[t_group].volume != QUIET) {
			  ...
			  if(g_tab[t_group].volume != QUIET) {
                ...
                mdb("topic: wrong number of parz");
        }
	return 0;
}
...
int s_status_group(int k, 
		int tellme, int n, char *class_string, char *message_string)
{
        int i;
        char my_group[MAX_GROUPLEN+1];
	...
	if (k==1) {
        	strcpy(my_group, u_tab[n].group);
		if (strlen(my_group) == 0) mdb("bad u_tab entry");
	} else {
        	strcpy(my_group, g_tab[n].name);
		if (strlen(my_group) == 0) mdb("bad g_tab entry");
	}
